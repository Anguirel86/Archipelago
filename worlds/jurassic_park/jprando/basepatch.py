...
def _patch_ap_item_handling(rom: JPRom):
    ...
    ow_hook: assemble.ASMList = [
        inst.JSL(rom.to_cpu_addr(ow_tramp_addr))
    ]
    rom.write(0x00834E, assemble.assemble(ow_hook))

    indoor_hook = assemble.ASMList = [
        inst.JSL(rom.to_cpu_addr(indoor_tramp_addr))
    ]
...
def _patch_id_card_pickup(rom: JPRom):
    ...
    id_pickup_hook: assemble.ASMList = [
...
def _patch_battery_pickup(rom: JPRom):
    ...
    rom.write_patch_with_padding(batt_loc_hook_addr,
                                 batt_loc_hook_addr + 13,
                                 assemble.assemble(batt_loc_hook))
    ...
    sprite_load_routine: assemble.ASMList = [
...
def _patch_nerve_gas(rom: JPRom):
    ...
    nerve_gas_hook: assemble.ASMList = [
        inst.JSL(rom.to_cpu_addr(patch_addr)),
        inst.NOP(),
        inst.NOP(),
        inst.NOP()
    ...
    ng_patch: assemble.ASMList = [
        inst.JMP(rom.to_cpu_addr(deploy_addr), AM.LNG),
        inst.NOP()
    ]
...
def _patch_egg_pickup(rom: JPRom, eggs_required: int):
    ...
    routine: assemble.ASMList = [
        inst.LDA(eggs_required, AM.IMM16)
    ]
...
def apply_AP_base_patch(rom: JPRom, player_name: str, eggs_required: int):
    """
