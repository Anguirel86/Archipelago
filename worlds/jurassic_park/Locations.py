...
from .jprando.types import MapId
...
class ConnectorData:
    ...
    access_tokens: typing.Optional[list[list[str]]]
    reversible: bool
    map_id: typing.Optional[MapId] = None
...
connector_table: dict[str, ConnectorData] = {
    "Nublar Utility Shed": ConnectorData(
        "Overworld Start", "Nublar Utility Shed", None, True, MapId.NUBLAR_UTILITY_SHED_ENTRY_LEVEL
    ),
    "Beach Utility Shed": ConnectorData(
        "Overworld Start", "Beach Utility Shed", None, True, MapId.BEACH_UTILITY_SHED_ENTRY_LEVEL
    ),
    "Visitor Center": ConnectorData("Overworld Start", "Visitor Center", None, True, MapId.VISITOR_CENTER_ENTRY_LEVEL),
    "Raptor Pen": ConnectorData("Overworld Start", "Raptor Pen", None, True, MapId.RAPTOR_PEN_ENTRY_LEVEL),
    "Ship": ConnectorData("Overworld Start", "Ship", None, True, MapId.SHIP_ENTRY_LEVEL),
    # These last two need the park systems to be rebooted
    # in order to open the gates
    "Raptor Nest": ConnectorData("Overworld Start", "Raptor Nest", [["Reboot Park Systems"]], True),
    "North Utility Shed": ConnectorData(
        "Overworld Start", "North Utility Shed", [["Reboot Park Systems"]], True, MapId.NORTH_UTILITY_SHED_ENTRY_LEVEL
    ),
...
location_table: dict[str, LocationData] = {
    ...
    "Behind Visitor Center Egg": LocationData(0x0F, [["Reboot Park Systems"]], "North Utility Shed"),
    "North Utility Shed Egg": LocationData(0x10, [["Reboot Park Systems"]], "North Utility Shed"),
    # Raptor nest egg locations
    "North Raptor Nest Egg": LocationData(0x11, [["Reboot Park Systems"]], "Raptor Nest"),
    "South Raptor Nest Egg": LocationData(0x12, [["Reboot Park Systems"]], "Raptor Nest"),
    #####################
    # ID Card locations #
    #####################
    ...
    "Ray Arnold ID Card": LocationData(
        0x25, [["Beach Utility Shed Battry", "Donald Gennaro ID Card"]], "Beach Utility Shed"
    ),
    "Dennis Nedry ID Card": LocationData(0x26, None, "Beach Utility Shed"),
    "Doctor Wu ID Card": LocationData(0x27, [["Ship Battery"]], "Ship"),
    "Ian Malcolm ID Card": LocationData(0x28, [["Raptor Pen Battery"]], "Raptor Pen"),
    #############
    # Batteries #
    #############
    "North Utility Shed Battery": LocationData(0x40, [["Reboot Park Systems"]], "North Utility Shed"),
    "Raptor Pen Battery": LocationData(0x41, None, "Raptor Pen"),
    "Visitor Center Battery": LocationData(0x42, [["John Hammond ID Card"]], "Visitor Center"),
    "Beach Utility Shed Battery": LocationData(0x43, [["Donald Gennaro ID Card"]], "Beach Utility Shed"),
    "Nublar Utility Shed Battery": LocationData(0x44, None, "Nublar Utility Shed"),
    "Ship Battery": LocationData(0x45, None, "Ship"),
    ###################
    # Event Locations #
    ###################
    "Turn on Generator": LocationData(0x60, None, "Nublar Utility Shed", True),
    "Reboot Park Systems": LocationData(0x61, [["Turn on Generator", "Dennis Nedry ID Card"]], "Visitor Center", True),
    "Activate Motion Sensors": LocationData(0x62, [["Reboot Park Systems"]], "Visitor Center", True),
    ...
    "Security Level 1": LocationData(0x64, [["Reboot Park Systems", "Doctor Wu ID Card"]], "Visitor Center", True),
    "Security Level 2": LocationData(0x65, [["Security Level 1"]], "Ship", True),
    "Contact Ship": LocationData(0x66, [["Security Level 1", "Block Raptor Pen Door"]], "Raptor Pen", True),
    "Clear Ship": LocationData(
        0x67, [["Ship Battery", "Security Level 2", "Ray Arnold ID Card", "Block Raptor Pen Door"]], "Ship", True
    ),
    "Collect Nerve Gas": LocationData(
        0x68, [["Reboot Park Systems", "Robert Muldoon ID Card"]], "North Utility Shed", True
    ),
    "Destroy Raptor Nest": LocationData(
        0x69, [["Reboot Park Systems", "Collect Nerve Gas", "Clear Ship"]], "Raptor Nest", True
    ),
    "Contact Mainland": LocationData(0x6A, [["Destroy Raptor Nest", "Ship Battery"]], "Ship", True),
}
...
def create_regions(player: int, multiworld: MultiWorld, eggs_required: int):
    ...
    for name, loc_data in location_table.items():
        ...
        loc = create_location(player, name, loc_data.id, loc_data.access_tokens, region)
    ...
    for name, conn_data in connector_table.items():
        ...
        exit_obj.access_rule = create_access_rule(player, conn_data.access_tokens)
        exit_obj.connect(to_region)

        # If the connector is reversible, add its opposite
        if conn_data.reversible:
            reverse_name = f"{conn_data.to_region_name}-{conn_data.from_region_name}"
            reverse_exit = to_region.create_exit(reverse_name)
            reverse_exit.access_rule = create_access_rule(player, conn_data.access_tokens)
...
def create_location(
    player: int, name: str, loc_id: int, rules: typing.Optional[list[list[str]]], region: Region
) -> JPLocation:
    """
...
def create_event_location(player: int, name: str, rules: typing.Optional[list[list[str]]], region: Region):
    access_rule = create_access_rule(player, rules)
    region.add_event(f"{name}_evt", name, access_rule, show_in_spoiler=False)


def create_victory_location(player: int, region: Region, eggs_required: int):
...
def create_access_rule(player: int, rules: typing.Optional[list[list[str]]]) -> Callable[[CollectionState], bool]:
...
def get_id_to_name_mapping() -> dict[int, str]:
    return {loc.id + JP_LOC_ID_BASE: name for name, loc in location_table.items()}


def get_name_to_id_mapping() -> dict[str, int]:
    return {name: loc.id + JP_LOC_ID_BASE for name, loc in location_table.items()}
